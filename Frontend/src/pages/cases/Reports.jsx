...
const Reports = () => {
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      (r.Report_Type && r.Report_Type.toLowerCase().includes(search)) ||
      ...
      <div className="pm-page" style={{padding:'2rem'}}>
        <div className="pm-page-header" style={{marginBottom:'2rem', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem'}}>
          <div>
            <div className="pm-breadcrumb">
              <Link to="/reports">Reports</Link><span>/</span><span>All Records</span>
            </div>
        ...
        <div className="pm-card">
          <div className="pm-card-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem'}}>
            <h2 className="pm-card-title"><ClipboardList size={18} style={{marginRight:'0.5rem'}}/> Report Directory</h2>
            <div className="pm-search-box" style={{position:'relative', width:'300px'}}>
              <Search size={16} style={{position:'absolute', left:'10px', top:'50%', transform:'translateY(-50%)', color:'#64748b'}}/>
              <input 
                type="text" 
                className="pm-input" 
                placeholder="Search Type or Doctor Name..." 
                value={searchTerm}
            ...
            <table className="pm-table" style={{width:'100%', minWidth:'800px'}}>
              <thead>
                <tr>
                  ...
                  <th style={{textAlign:'left'}}>Report Type</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredRecords.map(r => (
                    <tr key={r.Report_ID}>
                      <td style={{fontWeight:600}}>RPT-{r.Report_ID}</td>
