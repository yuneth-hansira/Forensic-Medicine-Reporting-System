...
const CaseList = () => {
  ...
  const filteredCases = cases.filter(c => {
    ...
    return (
      (c.MLEF_No_or_PM_No && c.MLEF_No_or_PM_No.toLowerCase().includes(search)) ||
      (c.Case_Type && c.Case_Type.toLowerCase().includes(search))
    );
  });
      ...
      <div className="pm-page" style={{padding:'2rem'}}>
        <div className="pm-page-header" style={{marginBottom:'2rem', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem'}}>
          <div>
            <div className="pm-breadcrumb">
              <Link to="/cases">Case Management</Link><span>/</span><span>All Cases</span>
            </div>
            <h1 className="pm-page-title">All Cases</h1>
        ...
        <div className="pm-card">
          <div className="pm-card-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem'}}>
            <h2 className="pm-card-title"><FileText size={18} style={{marginRight:'0.5rem'}}/> Case Directory</h2>
            <div className="pm-search-box" style={{position:'relative', width:'300px'}}>
              <Search size={16} style={{position:'absolute', left:'10px', top:'50%', transform:'translateY(-50%)', color:'#64748b'}}/>
              <input 
                type="text" 
                className="pm-input" 
                placeholder="Search by Ref No or Type..." 
                value={searchTerm}
            ...
            <table className="pm-table" style={{width:'100%', minWidth:'600px'}}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="6" style={{textAlign:'center', padding:'2rem', color:'#64748b'}}>No cases found.</td></tr>
                ) : (
                  filteredCases.map(c => (
                    <tr key={c.Case_ID}>
